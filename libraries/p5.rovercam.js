...
class RoverCam {
  constructor(instance) {
    ...
    this.keyMap = {
      // maps each control command to a pair of keys
  ...
  controller() {
    ...
    if (RoverCam.pointerLock) {
      ...
      if (p.keyIsDown(k.e1[0]) || p.keyIsDown(k.e1[1]))
        this.elevate(-this.speed); // r
      if (p.keyIsDown(k.e2[0]) || p.keyIsDown(k.e2[1]))
        this.elevate(this.speed); // f
    } else {
      // otherwise yaw/pitch with keys
      if (p.keyIsDown(k.y1[0]) || p.keyIsDown(k.y1[1]))
        this.yaw(-this.sensitivity); // a
      if (p.keyIsDown(k.y2[0]) || p.keyIsDown(k.y2[1]))
        this.yaw(this.sensitivity); // d
      if (p.keyIsDown(k.p1[0]) || p.keyIsDown(k.p1[1]))
        this.pitch(-this.sensitivity); // r
      if (p.keyIsDown(k.p2[0]) || p.keyIsDown(k.p2[1]))
        this.pitch(this.sensitivity); // f
    }
    ...
    if (p.keyIsDown(k.f1[0]) || p.keyIsDown(k.f1[1]))
      this.fov(-this.sensitivity / 10); // +
    if (p.keyIsDown(k.f2[0]) || p.keyIsDown(k.f2[1]))
      this.fov(this.sensitivity / 10); // -
  ...
  setActive(active) {
    // method to switch between multiple cameras
    this.active = active;
    if (active) this.width = 0; // trigger a perspective call in the draw loop
  }
  setState(state) {
    // state object can have fov,active,rotation,position
    ...
    if (state.enableControl !== undefined)
      this.enableControl = state.enableControl;
  ...
  draw() {
    ...
    this.forward = new p5.Vector(
      Math.cos(this.pan),
      Math.tan(this.tilt),
      Math.sin(this.pan)
    );
    this.forward.normalize();
    this.right = new p5.Vector(
    ...
    let position = p5.Vector.sub(
      this.position,
      p5.Vector.mult(this.right, this.offset[1])
    );
    let center = p5.Vector.add(position, this.forward);
    this.p5.camera(
      position.x,
      position.y + this.offset[0],
      position.z,
      center.x,
      center.y + this.offset[0],
      center.z,
      this.up.x,
      this.up.y,
      this.up.z
...
RoverCam.onPointerlockChange = () => {
  if (
    document.pointerLockElement !== RoverCam.canvas &&
    document.mozPointerLockElement !== RoverCam.canvas
  )
    RoverCam.pointerLock = false;
};
